...
def test_dsa_problem_parsing_wrapped_json():
    from interviewos.llm.structured_output import parse_structured_output
    
    # Nested wrapper output commonly generated by LLMs
    raw_response = '''
    {
        "dsa_problem": {
            "title": "Two Sum",
            ...
            "hidden_solution_information": "Use a hash map to store complements."
        }
    }
    '''
    
    problem = parse_structured_output(raw_response, DSAProblem)
    assert problem.title == "Two Sum"
    assert problem.difficulty == "medium"
    assert problem.expected_complexity == "O(N) time, O(N) space"
