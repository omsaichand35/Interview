...
from interviewos.interview.session import InterviewSession, InterviewType, InterviewDecision, AnswerAssessment, DifficultyChange, Misconception, DepthLevel
...
async def test_technical_interview_integration(mock_llm_client):
    ...
    session = InterviewSession(id="tech1", interview_type=InterviewType.TECHNICAL, candidate_id="c1", job_id="j1", duration_minutes=30)
    ...
    decision2 = await engine.process_answer(context, "Oh right, backward() computes gradients, step() updates weights.")
