...
from interviewos.models.plan import (
    AssessmentRecord,
    PreparationPlan,
    Priority,
    TopicNode,
...
class LearningPlanUpdater:
    ...
    def update_plan(
        self, plan: PreparationPlan, gaps: list[LearningGap]
    ) -> PreparationPlan:
        ...
        for gap in gaps:
            ...
            notes = f"Identified in {gap.source_round} interview. " + " ".join(
                gap.evidence
            )
        ...
        def _format_group(group_name, node_list, start_idx=1):
            ...
            for n in node_list:
                ...
                if n.assessment_history:
                    latest = max(n.assessment_history, key=lambda a: a.timestamp)
